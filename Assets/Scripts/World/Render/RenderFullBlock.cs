...
public class RenderFullBlock : IRenderBlock
{
    ...
    public void Render (MeshBuildInfo current, World world, Vector2i colPos, int x, int y, int z)
    {
        bool smooth = true;
        ...
        if (!Block.GetInstance (world.GetBlockAt (colPos, x, y + 1, z, def)).Opaque) {
            if (smooth) {
                for (int zz = -1; zz <= 1; zz++) {
                    for (int xx = -1; xx <= 1; xx++) {
                        l [3 * (zz + 1) + (xx + 1)] = world.GetLightAt (colPos, xx + x, y + 1, -zz + z, 0);
                    }
                }
            } else {
                l [4] = world.GetLightAt (colPos, x, y + 1, z, 0);
            }
        ...
        if (!Block.GetInstance (world.GetBlockAt (colPos, x + 1, y, z, def)).Opaque) {
            if (smooth) {
                for (int zz = -1; zz <= 1; zz++) {
                    for (int yy = -1; yy <= 1; yy++) {
                        l [3 * (zz + 1) + (yy + 1)] = world.GetLightAt (colPos, x + 1, yy + y, -zz + z, 0);
                    }
                }
            } else {
                l [4] = world.GetLightAt (colPos, x + 1, y, z, 0);
            }
        ...
        if (!Block.GetInstance (world.GetBlockAt (colPos, x, y, z + 1, def)).Opaque) {
            if (smooth) {
                for (int yy = -1; yy <= 1; yy++) {
                    for (int xx = -1; xx <= 1; xx++) {
                        l [3 * (yy + 1) + (xx + 1)] = world.GetLightAt (colPos, xx + x, -yy + y, z + 1, 0);
                    }
                }
            } else {
                l [4] = world.GetLightAt (colPos, x, y, z + 1, 0);
            }
