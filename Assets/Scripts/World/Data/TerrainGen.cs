    ...
    private static void BasicTerrain(Column col, int xOffset, int zOffset)
    {
        ...
        for (int x = 0; x < World.CHUNK_SIZE; x++)
        {
            for (int z = 0; z < World.CHUNK_SIZE; z++)
            {
                ...
                double worldScale = 100;
                double mountains = 1 + PerlinNoise(x + xOffset, -1000, z + zOffset, 1500, 3, 2.8);
                for (int i = 0; i < stoneLayers; i++)
                {
                    int height = worldHeight / (stoneLayers + 1) / 4;
                    stoneHeights[i] = (height * (i + 1)) + PerlinNoiseI(x + xOffset, height, z + zOffset, worldScale, 6 * mountains, 1);
                }
                int dirtHeight = stoneHeights[stoneLayers - 1] + 3 + PerlinNoiseI(x + xOffset, 100, z + zOffset, worldScale/2, 3, 1);
